...
public class TcpSpeech : MonoBehaviour
{
	...
	void Start ()
	{
        ...
        if (Application.loadedLevelName.Equals("Demo"))
        {
            msg = new JSONObject();
            msg.AddString("type", "command");
            msg.AddString("command", "AddGrammar");
            msg.AddString("name", "demo");
            msg.AddString("filename", "demo");
            connector.Message(msg, false);
            msg.SetString("command", "LoadGrammar");
            connector.Message(msg, false);
        }
	}
